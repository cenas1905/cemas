...
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || 'dummy-gemini-key');
const anthropic = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY || 'dummy-anthropic-key',
});
...
function getMockResponse(prompt: string): string {
  ...
  if (prompt.includes('İngilizceye çevir') || prompt.includes('translate-cv')) {
    return JSON.stringify({
      personal: { fullName: "Ahmet Yılmaz", headline: "Software Engineer", location: "Istanbul, Turkey", email: "ahmet@example.com", linkedin: "linkedin.com/in/ahmet", summary: "Experienced software developer with modern web technologies." },
      experience: [{ title: "Software Engineer", company: "Tech Company", startDate: "2021", endDate: "Present", current: true, description: "Developed large scale web applications using Next.js.", location: "Istanbul" }],
      education: [{ school: "Bogazici University", degree: "Bachelor's", field: "Computer Engineering", startYear: "2016", endYear: "2020" }],
      skills: ["React", "Node.js", "TypeScript", "Next.js", "Tailwind CSS"],
      certifications: [{ name: "AWS Certified Developer", issuer: "Amazon Web Services", date: "2023" }]
    });
  }
...
// CV'yi İngilizceye Çevir
export async function translateCV(cvData: any) {
  const prompt = `Aşağıdaki CV verisini tamamen profesyonel iş İngilizcesine çevir.
  
CV Verisi:
${JSON.stringify(cvData, null, 2)}

Sadece JSON formatında İngilizce CV'yi döndür. Alan adlarını (keys) aynı bırak (personal, experience vb.), sadece içerikleri İngilizceye çevir. JSON nesnesi şu alanları içermelidir: personal (fullName, headline, location, email, linkedin, summary, photo), experience (title, company, startDate, endDate, current, description, location dizisi), education (school, degree, field, startYear, endYear dizisi), skills (string dizisi), certifications (name, issuer, date dizisi). Cevabında hiçbir açıklama ya da giriş cümlesi olmasın, sadece saf JSON verisi döndür.`;

  try {
    const text = await ask(prompt);
    return JSON.parse(extractJSON(text));
  } catch (e) {
    console.error('translateCV parse/API error:', e);
    throw e;
  }
}
